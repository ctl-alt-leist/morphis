...
class TestBladeSpec:
    ...
    def test_geometric_shape_vector(self):
        ...
        spec = BladeSpec(grade=1, collection_dims=1, dim=3)
    ...
    def test_geometric_shape_trivector(self):
        ...
        spec = BladeSpec(grade=3, collection_dims=1, dim=4)
    ...
    def test_blade_shape_multiple_collection(self):
        ...
        spec = BladeSpec(grade=1, collection_dims=2, dim=4)

        shape = spec.blade_shape((5, 10))
        assert shape == (5, 10, 4)

    def test_blade_shape_wrong_collection_dims(self):
    ...
    def test_negative_collection_dims_raises(self):
        """Test that negative collection_dims raises ValueError."""
        with pytest.raises(ValueError, match="collection_dims must be non-negative"):
            BladeSpec(grade=1, collection_dims=-1, dim=3)

    def test_zero_dim_raises(self):
        """Test that zero dim raises ValueError."""
        with pytest.raises(ValueError, match="dim must be positive"):
            BladeSpec(grade=1, collection_dims=1, dim=0)
    ...
    def test_grade_exceeds_dim_raises(self):
        ...
        with pytest.raises(ValueError, match="grade 4 cannot exceed dim 3"):
            BladeSpec(grade=4, collection_dims=1, dim=3)


class TestBladeSpecHelper:
    """Tests for blade_spec helper function."""

    def test_default_collection_dims(self):
        """Test that blade_spec defaults to collection_dims=1."""
        spec = blade_spec(grade=2, dim=3)

        assert spec.collection_dims == 1

    def test_explicit_collection_dims(self):
        """Test blade_spec with explicit collection_dims."""
        spec = blade_spec(grade=2, dim=3, collection_dims=0)

        assert spec.collection_dims == 0
