...
def basis_vector(idx: int, metric: Metric) -> Blade:
    ...
    data[idx] = 1.0
    return vector_blade(data, metric)
...
def demo_geometric_basics() -> None:
    ...
    u = vector_blade(array([1.0, 0.0, 0.0]), g)  # e_1
    v = vector_blade(array([0.0, 1.0, 0.0]), g)  # e_2
    ...
    v = vector_blade(array([3.0, 0.0, 0.0]), g)  # v = 3u
    ...
    theta = pi / 4
    u = vector_blade(array([1.0, 0.0, 0.0]), g)
    v = vector_blade(array([cos(theta), sin(theta), 0.0]), g)
...
def demo_vector_contraction() -> None:
    ...
    v = vector_blade(array([3.0, 4.0, 0.0]), g)
    v_sq = geometric(v, v)
...
def demo_inverse() -> None:
    ...
    v = vector_blade(array([3.0, 4.0, 0.0]), g)  # |v|^2 = 25
    v_inv = inverse(v)
